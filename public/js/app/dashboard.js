...
function initializeDashboardCharts() {
    if (window.dashboardOverviewChartData) {
        createOverviewChart({
            chartId: 'chart-weekly-revenue',
            chartData: window.dashboardOverviewChartData.weeklyRevenue,
            color: '#206bc4',
            seriesName: 'Revenue'
        });

        createOverviewChart({
            chartId: 'chart-weekly-profit',
            chartData: window.dashboardOverviewChartData.weeklyProfit,
            color: '#2fb344',
            seriesName: 'Profit'
        });

        createOverviewChart({
            chartId: 'chart-weekly-customers',
            chartData: window.dashboardOverviewChartData.weeklyCustomers,
            color: '#d6336c',
            seriesName: 'Pelanggan'
        });
    } else {
        console.warn('dashboardOverviewChartData not found on window.');
    }
}

/**
 * Helper function to create a new ApexChart for the overview cards.
 * @param {object} options - The configuration for the chart.
 * @param {string} options.chartId - The ID of the HTML element for the chart.
 * @param {object} options.chartData - The data object for the chart series.
 * @param {string} options.color - The primary color for the chart series.
 * @param {string} options.seriesName - The name for the data series.
 */
function createOverviewChart({ chartId, chartData, color, seriesName }) {
    const element = document.getElementById(chartId);
    if (!element || !chartData || !chartData.values || chartData.values.length < 2) {
        if (element) {
            element.innerHTML = `<div class="text-muted text-center" style="padding-top:10px;">${chartData.error || 'Data tidak cukup'}</div>`;
        }
        return;
    }
    ...
    const options = {
        ...
        tooltip: {
            ...
            x: {
                formatter: function(value, { dataPointIndex, w }) {
                    return w.globals.categoryLabels[dataPointIndex] || value;
                }
            },
            y: {
                formatter: function(value) {
                    if (chartId.includes('customers')) {
        ...
        responsive: [{
            ...
            options: {
                xaxis: {
                    tickAmount: Math.floor((chartData.labels || []).length / 2)
                }
            },
        }, {
            breakpoint: 480,
            options: {
                xaxis: {
                    tickAmount: Math.floor((chartData.labels || []).length / 3)
                }
            }
        }]
    };
